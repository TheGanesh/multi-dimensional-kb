# 8 — The milk carton principle (keeping it fresh)
...
card in [Chapter 3](./03-addresses-and-metadata.md)). Feels safe, right? Expired page →
warning label → done?
...
![The freshness model](./images/freshness-model.svg)
...
| **2. RESTOCK** (refresh) | Only the changed services get re-ingested (that's the LLM re-reading their code), producing an auto-PR that must pass *every* gate from [Chapter 6](./06-daily-operations.md) before merging. | Same night |
...
it yet" discipline gets a whole chapter — [Chapter 11](./11-less-is-more.md).)
...
## Under the Hood — detect, refresh, backstop

The machinery, precisely — everything runs inside the domain KB's own CI, touching
nothing owned by other teams:
...
|-------|-----------|----------------|------|
| **Detect** | Nightly scan: pull the `seed.yaml`-declared repos, hash ingest-relevant files, diff `.fingerprints.json` (which already exists for exactly this — manifest v2: flat source-file hashes plus per-service `last_ingested`, `chunk_count`, source hash). Zero changes to service repos | Change *known* ≤ 24 h | ~$0 (git pulls + hashing) |
...
| **Backstop** | `stale_after` + weekly audit — it should *rarely fire*; when it does, it means detect/refresh broke (e.g., the scan job failed silently — which is why the scan job alerts on failure) | Catch-all | $0 |

### Freshness SLOs (published per domain)
...
These three numbers, in the weekly usage report, are the answer to *"how do we know the
common KB stays current?"* Once multiple domains exist, each domain's SLOs are declared
in `registry.yaml` and checked by federation CI ([Chapter 10](./10-federation.md)) —
violations notify that domain's owner, not a shared inbox nobody reads.

### The rejected event-driven variant, on the record

A CI hook in every service repo notifying the KB on merge was evaluated and **cut**:
it would require changes to 45 repos owned by other teams to accelerate *detection* —
which is not the bottleneck, since *ingestion* runs nightly either way. Nightly
detection gives ≤ 24 h against a ≤ 7-day SLO — 7× headroom, zero cross-team footprint.

**Restore condition** (written down, not vibes): the SLO tightens to same-day, or the
scan repeatedly misses SLO.
...
- Freshness has *numbers*: ≤ 24 h typical, ≤ 7 days promised, ≥ 95% of pages within
  TTL — and the backstop pages a human when the machinery itself breaks.
...
**Next**: [9 — Trust, but verify](./09-evaluation.md) | **Back**: [7 — Botty gets a job](./07-consumers.md)
